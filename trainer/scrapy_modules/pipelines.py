...
class MongoDBPipeline(object):
    ...
    def open_spider(self,spider):
      if self.settings['DATABASE_ENABLED']:
        ...
        logger.info("Establishing connection to mongoDB %s:%s" % (self.settings['TRAINER_MONGODB_SERVER'], self.settings['TRAINER_MONGODB_PORT']))
        self.connection = pymongo.MongoClient(self.settings['TRAINER_MONGODB_SERVER'], self.settings['TRAINER_MONGODB_PORT'])
        self.db = self.connection[self.settings['TRAINER_MONGODB_DB']]
        self.collection = self.db[self.settings['TRAINER_MONGODB_COLLECTION']]
        if self.__get_uniq_key() is not None:
            try:
              ...
              logger.error("Unable to connect to mongoDB server %s. Error: %s" %(self.settings['TRAINER_MONGODB_SERVER'],e))
            except Exception as e:
              logger.error("An error occured. Error: %s" %(e))
    
    def close_spider(self,spider):
      self.connection.close()
      logger.info("Closed connection to mongoDB %s:%s" % (self.settings['TRAINER_MONGODB_SERVER'], self.settings['TRAINER_MONGODB_PORT']))
    ...
    def process_item(self, item, spider):
        ...
        if self.settings['DATABASE_ENABLED']:
          if self.__get_uniq_key() is None:
              try:
                logger.debug("Attempting insert new record for %s %s" % (self.settings['TRAINER_MONGODB_UNIQ_KEY'],item[self.__get_uniq_key()]))
              ...
              if existing is not None:
                logger.debug("Attempting update record for %s %s" % (self.settings['TRAINER_MONGODB_UNIQ_KEY'],item[self.__get_uniq_key()]))
    ...
    def __get_uniq_key(self):
        if not self.settings['TRAINER_MONGODB_UNIQ_KEY'] or self.settings['TRAINER_MONGODB_UNIQ_KEY'] == "":
            return None
        return self.settings['TRAINER_MONGODB_UNIQ_KEY']
